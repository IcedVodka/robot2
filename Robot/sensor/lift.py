import serial
import time
import struct

class SerialLiftingMotor: 
    def __init__(self):
        self.ser = None  # 串口对象
        self.serial_opened = False  # 串口状态标志

        self.init_serial("/dev/ttyACM0",19200)
        self.motor_init()
        self.remaining_data = b''  # serial buffer left 数据缓冲区
        #其中数值 3276800 表示电机转动100圈，电机转动1圈的数值是32768.
        self.max_lifting_distance = 700
        self.robot_name = "ADORA_A2_MAX"
        self.Ratio_K_1 = 0.63636364 #// 电机每转动一圈丝杆行进的距离（单位 mm ） 
        self.Ratio_K_2 = 32768 # 电机转动1圈的编码器反馈的数值是32768

        self.MIN_MOTOR_POSITION = 0
        self.MAX_MOTOR_POSITION = 36044800  #//勿修改！！！ 32768*1100(圈) = 36044800 对应700mm行程的丝杠，
        self.motor_positon_read = 0
        self.motor_position_set(0)
        # 配置串口参数
    def init_serial(self, port, baudrate=115200):
        try:
                # 初始化串口，设置串口参数
            self.ser = serial.Serial(
                port=port,  # 串口名称
                baudrate=baudrate,  # 波特率
                bytesize=serial.EIGHTBITS,  # 数据位8位
                parity=serial.PARITY_NONE,  # 校验位，无校验位
                stopbits=serial.STOPBITS_ONE,  # 停止位1位
                timeout=0.1  # 超时时间
        )
        except Exception as e:
            print("串口初始化失败:", e)

    def run(self):
        # while True:
        self.motor_position_read()
        time.sleep(0.1)

        # 读取数据并拼接剩余数据
        self.remaining_data += self.ser.read_all()
        print("self.remaining_data:", len(self.remaining_data))
        buffer = self.remaining_data

        # 查找数据头
        start_index = buffer.find(b'\x01\x03\x04')
        if start_index != -1 and len(buffer) >= start_index + 9:
            # 取出九个字节
            data_segment = buffer[start_index:start_index + 9]
            self.remaining_data = buffer[start_index + 9:]  # 更新剩余数据

            # 解析脉冲数
            position_bytes = bytearray()
            position_bytes.extend([data_segment[5], data_segment[6], data_segment[3], data_segment[4]])  # 高位存低地址
            self.motor_positon_read = int.from_bytes(position_bytes, 'big', signed=True)
            print("position pulse count:", self.motor_positon_read)

            # 转换为距离
            self.motor_positon_read = int(self.motor_positon_read / self.Ratio_K_2 * self.Ratio_K_1) / 1
            print("position distance (mm):", self.motor_positon_read, flush=True)
        else:
            print("no data")




    def get_motor_poition(self)->int:
        return self.motor_positon_read


    def motor_init(self):
        data_array_1 = bytearray()
        data_array_1.extend([0x01, 0x06, 0x00, 0x00, 0x00, 0x01, 0x48, 0x0A])  # enable modbus

        data_array_2 = bytearray()
        data_array_2.extend([0x01, 0x06, 0x00, 0x01, 0x00, 0x01, 0x48, 0xCA])  # enable motor

        data_array_3 = bytearray()
        data_array_3.extend([0x01, 0x06, 0x00, 0x03, 0x13, 0x88, 0x74, 0x9C])  # accel speed 5000

        data_array_4 = bytearray()
        # data_array_4.extend([0x01, 0x06, 0x00, 0x02, 0x05, 0xDC, 0x2A, 0xC3])  # speed 1500
        data_array_4.extend([0x01, 0x06, 0x00, 0x02, 0x07, 0xD0, 0x2A, 0xC3])  # speed 2000
        
        if not self.ser.is_open:
            self.ser.open()
            
        # 发送数据
        self.ser.write(data_array_1)
        time.sleep(0.2)
        self.ser.write(data_array_2)
        time.sleep(0.2)
        self.ser.write(data_array_3)
        time.sleep(0.2)
        self.ser.write(data_array_4)
        time.sleep(0.2)

    def calculate_crc16(self,data: bytes) -> bytes:
        auchCRCHi = [ # CRC 高位字节值表 */
            
        ]
        #  CRC 低位字节值表*/
        auchCRCLo = [
            
        ]
    
        
        crc_hi = 0xFF  # 高字节初始化
        crc_lo = 0xFF  # 低字节初始化
        
        for byte in data:
            # 计算索引（修正原代码中的 *puchMsgg++ 错误）
            uIndex = crc_hi ^ byte
            # 更新CRC值
            temp = crc_hi
            crc_hi = crc_lo ^ auchCRCHi[uIndex]
            crc_lo = auchCRCLo[uIndex]
        
        # 返回组合后的CRC值（注意高位在前）
        return bytes([crc_hi, crc_lo])
    
    # 读取电机位置
    def motor_position_read(self):
        data_array_1 = bytearray()
        # 01030016000225CF
        data_array_1.extend([0x01, 0x03, 0x00, 0x16, 0x00, 0x02])  # read motor global position
        crc_values = self.calculate_crc16(data_array_1) # calculate crc
        data_array_1.extend([crc_values[0], crc_values[1]])
        #print(f"CRC16: 0x{data_array_1.hex().upper()}")
        # print(f"Query position cmd: CRC16: 0x{data_array_1.hex().upper()}",flush=True)

        
        if not self.ser.is_open:
            self.ser.open()
        # 发送数据
        self.ser.write(data_array_1)
        self.ser.flush()
            

    #  电机M8010HG8 额定转速2500RPM (加速比8    0-277RPM)  4NM 的型号
    # 丝杠起点位置： 0 
    # 丝杠终点位置： 32768*1100(圈) = 36044800 对应700mm行程的丝杠，
    # 其中32768是电机外部的转轴转动一圈的值。
    def motor_position_set(self,position_value : float):

        data_array_1 = bytearray()
        data_array_1.extend([0x01, 0x7b, 0x00, 0x00, 0x00, 0x00])  # set motor position

        if(position_value < self.MIN_MOTOR_POSITION):
            position_value = self.MIN_MOTOR_POSITION 
        elif(position_value > self.MAX_MOTOR_POSITION):
            position_value = self.MAX_MOTOR_POSITION
        # 取整
        position_value = int(position_value)
        
        # 将position_value转换为 大端序 有符号32位整数
        data_array_1[2:6] = struct.pack('>i', position_value)

        print(f"CRC16: 0x{data_array_1.hex().upper()}")
        # calculate crc
        crc_values = self.calculate_crc16(data_array_1)
        data_array_1.extend([crc_values[0], crc_values[1]])
        if not self.ser.is_open:
            self.ser.open()
            
        # 发送数据
        self.ser.write(data_array_1)

    def cmd_vel_callback(self, distance:int):
        """速度命令回调函数：将目标距离转换为电机编码器脉冲数
        根据输入的目标提升距离，计算对应的电机编码器脉冲数，并确保距离在安全范围内。
        包含行程限制保护和脉冲数转换逻辑。

        Args:
            distance: 目标提升距离 (单位: mm)

        Note:
            - 当输入距离超过最大行程时，自动限制为最大行程减5mm(预留安全距离)
            - 脉冲数计算公式: encoder_pulse_count = distance / Ratio_K_1 * Ratio_K_2
            - Ratio_K_1/Ratio_K_2为丝杠传动比相关参数，用于距离到脉冲的转换
        """
        # 预留5mm安全行程，防止超出机械限位
        if distance > self.max_lifting_distance:
            distance = self.max_lifting_distance - 5 

        # 计算编码器脉冲数：距离(mm) -> 电机脉冲数转换
        encoder_pulse_count : float = distance / self.Ratio_K_1 * self.Ratio_K_2
        print("cmd distance (mm): %d, encoder_pulse_count: %.2f\n" % (distance, encoder_pulse_count))
        self.motor_position_set(encoder_pulse_count)

    def float_to_uint32(self,f):
        # 将浮点数打包为 IEEE 754 单精度格式（4字节）
        packed = struct.pack('>f', f)
        # 解包为 uint32
        return struct.unpack('>I', packed)[0]

if __name__ == "__main__":
    app = SerialLiftingMotor()
    app.run()
    app.cmd_vel_callback(400)
    # while True:
    #     app.cmd_vel_callback(200)
    #     time.sleep(20)   
    #     app.cmd_vel_callback(0)
    #     time.sleep(20)